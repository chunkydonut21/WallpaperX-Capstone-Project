...
public class DrawerFragment extends Fragment implements LoaderManager.LoaderCallbacks<List<Walls>> {

    private static String wallpapersUrl =
            "https://wall.alphacoders.com/api2.0/get.php?auth=" + BuildConfig.MY_API_KEY;
        ...
        floatingNext.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                ...
                if (number > 30) {
                    ...
                    getActivity().getLoaderManager().restartLoader(1, null, DrawerFragment.this);
    ...
    public Loader<List<Walls>> onCreateLoader(int id, Bundle args) {
        ...
        if (getArguments().containsKey("BundleWalls")) {

            methodParameter = getArguments().getString("BundleWalls");
            Uri baseUri = Uri.parse(wallpapersUrl);
            Uri.Builder ub = baseUri.buildUpon();
            ub.appendQueryParameter("method", methodParameter);
            ub.appendQueryParameter("page", pageNumber);
            return new WallpaperLoader(getContext(), ub.toString());

        } else if (getArguments().containsKey("BundleCategory")) {
            methodParameter = "category";
            String idParameter = getArguments().getString("BundleCategory");
            Uri baseUri = Uri.parse(wallpapersUrl);
            Uri.Builder ub = baseUri.buildUpon();
            ub.appendQueryParameter("method", methodParameter);
            ub.appendQueryParameter("id", idParameter);
            ub.appendQueryParameter("page", pageNumber);
            return new WallpaperLoader(getContext(), ub.toString());
        } else {

            String pokemon = getArguments().getString("BundleSearch");

            Uri baseUri = Uri.parse(wallpapersUrl);
            Uri.Builder ub = baseUri.buildUpon();
            ub.appendQueryParameter("method", "search");
            ub.appendQueryParameter("term", pokemon);
            return new WallpaperLoader(getContext(), ub.toString());
        }
